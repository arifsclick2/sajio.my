      ...
      <div className="relative w-full max-w-md rounded-3xl border border-stone-200 bg-white p-8 shadow-2xl shadow-brand-900/10">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          <div>
            ...
            <input id="email" type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} className={inputCls} placeholder="anda@restoran.my" />
          </div>
          <div>
            <label htmlFor="password" className="mb-1.5 block text-sm font-bold text-stone-700">Kata laluan</label>
        ...
        <p className="mt-8 text-center text-sm text-stone-500">
          Baru di Sajio?{" "}
          <Link href="/register" className="font-black text-brand-700 transition hover:text-brand-800">Cuba 14 hari percuma</Link>
        </p>
      </div>
    </main>
  );
